...
app_memory_usage_bytes = Gauge(
    ...
    'Использование памяти приложением в байтах'
)
...
class MetricsCollector:
    ...
    def record_telegram_command(self, command: str, user_id: str, success: bool, duration: float):
    ...
    def record_homeassistant_request(self, method: str, endpoint: str, status_code: int, duration: float):
        ...
        homeassistant_api_duration.labels(
            method=method,
            endpoint=endpoint
        ).observe(duration)
    ...
    def record_device_command(self, entity_id: str, command: str, success: bool, duration: float):
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_commands': sum([
                ...
                for cmd in ['start', 'help', 'status', 'lights', 'switches', 'sensors']
                for uid in ['all']
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                status_code = 500
                logger.error(f"Error in Home Assistant request {method} {endpoint}: {e}")
                raise
            finally:
                duration = time.time() - start_time
